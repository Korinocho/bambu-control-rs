...
**MQTT over the anchored stream (issue #1, option 2a).**

- **One connection loop, two callers, landing together.** `PrinterClient::start` (production) and `subscribe_gcode_state` (the `#[cfg(test)]` idle probe) both drive the same loop. **Correction to the earlier plan (owner, 2026-09-17):** this was to land probe-first, on the grounds that the probe was the only one of the two running against real hardware. That was wrong. `PrinterClient::start` runs on every app launch; the probe runs only during live tests. Production is the larger exposure by far, so the ordering had no safety argument behind it — and splitting a shared loop across two commits would have left that loop dead in release builds for one step, buying nothing.
- **Transport.** `PrinterTls::new(serial)` → `config_for_new_session()` → `AnchoredConnector::new(config, SessionConns::new(), io_timeout)` → `connect_stream(ip, socket)`. The handshake, both signatures and the CN check all complete before CONNECT is written, and CONNECT is what carries the access code. The property and its tests are therefore the camera's: zero TLS records of content type 23 reach a peer whose certificate was refused, and the connection record says `Refused`.
- **Codec.** rumqttc's `mqttbytes` is a `pub mod` and is re-exported unconditionally (`lib.rs:111` and `:141`), unlike every TLS item beside it, which is feature-gated. So `default-features = false` with no TLS feature at all still gives `Packet::read` / `Packet::write`.

  **Measured after the manifest change, 2026-09-17** (the owner's condition: measure after the line lands, never predict before it). What left: `native-tls`, `tokio-native-tls`, `schannel`, `openssl` + `openssl-sys` + `openssl-macros` + `openssl-probe`, `security-framework` + `security-framework-sys`, `foreign-types` + `foreign-types-shared`, `vcpkg` — 12 crates, 128 lines out of `Cargo.lock`. Both CI gates pass: no duplicate among rustls / rustls-webpki / ring / aws-lc-rs / aws-lc-sys, and `cargo tree -i` reports "did not match any packages" for both aws-lc crates.

  **What did NOT leave: tokio.** An earlier version of this row, of the `Cargo.toml` comment and of commit `6616820`'s message claimed 2a removes tokio along with native-tls. That was inferred and it is false. rumqttc declares `tokio`, `tokio-stream` and `tokio-util` as **non-optional** dependencies — unlike `native-tls`, `tokio-native-tls`, `tokio-rustls`, `rustls-webpki`, `rustls-native-certs` and `rustls-pemfile`, every one of which is `optional = true`. So tokio arrives by three unconditional paths and brings `mio`, `socket2`, `pin-project-lite` and `tokio-macros` with it. Keeping rumqttc for its codec means keeping the async runtime; only vendoring `mqttbytes` alone would change that, which is not planned. The decision for 2a stands on the `Resumption::disabled()` trap and the second-vendored-crate cost (10.5); the acceptance criterion "native-tls leaves the tree" is met and measured.
- **`bytes = "1"` becomes a direct dependency,** because `BytesMut` is not reachable through rumqttc: `mqttbytes/mod.rs` re-exports only `topic::*`. It resolves to the 1.12.1 already in `Cargo.lock`, so no crate and no version changes — but `cargo tree -d` and the CI dependency gate are re-run **after** the line is added, not predicted before it.
- **Keep-alive is in the shared loop from the start.** The probe's wait window can exceed the negotiated 30 s, and a probe that drops its connection for a missing PINGREQ is the worst possible ending for something whose entire job is to report whether a printer is idle.
- **SUBSCRIBE must be acknowledged.** `Subscribe::new` leaves `pkid: 0` (`subscribe.rs:19`) because rumqttc's `MqttState` assigns the real identifier downstream; hand-rolling the loop means assigning it here. A packet identifier of 0 is illegal for SUBSCRIBE in MQTT 3.1.1 and a broker may discard such a packet silently. The loop therefore assigns a non-zero pkid **and** requires a SUBACK carrying that same identifier **and** granting the subscription. A SUBACK may acknowledge while granting nothing (return code 0x80), which reads as success to anything that only checks whether one arrived, so the check is three-part: the identifier matches, `return_codes` is non-empty, and every code is `Success(_)`.

**As built (issue #1, 2026-09-17).** Seven tests, each verified to fail when its own property is removed and only then: the identifier is non-zero; a withheld SUBACK is an error and never "subscribed"; a SUBACK for another identifier is an error; a SUBACK granting nothing is an error; a healthy idle connection outlives three keep-alive intervals on its own pings; the probe reaches the loop with production's parameters; and a PINGRESP arriving while waiting for a SUBACK does not end the wait.

Two findings from that verification are worth keeping, because both were invisible while every test was green:

- **The deadline and the retry are separate properties, and for a while only one was tested.** `Session::attempt` is a single bounded try; `subscribe`'s own loop retries until the deadline. With the fast profile's 600 ms budget against a 200 ms ping interval, `attempt` returned `NoSubAck` before any PINGRESP arrived, so the `PingResp => {}` arm never executed once. A mutation making a PINGRESP abort the subscribe left all six tests green — the only survivor of nine mutations. `a_pingresp_while_waiting_for_a_suback_does_not_end_the_wait` closes it, with a wider deadline so pings must be consumed and looped past.
- **clippy found two correctness bugs the mutations did not.** `attempt` was twice written as a loop whose every arm returns (`loop`, then `while`), caught both times by `clippy::never_loop`; its comment claimed the deadline was enforced inside the wait while the code in fact relied on the ping interval, and a test passed on that accident. The general rule, which belongs beside the rest of the verification discipline: **a mutation must target the claim, not the text just written.** Two weak spots here came from mutating what was in front of the author — a parity assertion comparing `Timing::PRINTER` with itself, and a "fix" that was the same program in different syntax.

**What the idle probe may conclude, and what it may not.** The probe may report "idle" only on positive evidence, never from an absence of data. Silence is not idleness.

- This records existing behaviour rather than introducing a rule. `idle_from` refuses on `!connected` and on `reports == 0` before examining anything else, and its own comment already ends "Anything unknown is a refusal"; `subscribe_gcode_state`'s contract says a missing `gcode_state` is "state unknown", never "not printing".
- It follows that the `pkid: 0` defect would have failed **safe**, not catastrophically: a discarded SUBSCRIBE produces no reports, `reports == 0`, and the gate refuses the live test. Requiring the SUBACK is a fix to *diagnosis*, not to the gate. Without it the failure reads as "the printer sent no report at all", which invites the reading "the printer is quiet, use a longer window" when the truth is "we never subscribed".
...
**Out of the MVP, tracked:** MQTT (issue #1) and the camera (issue #2), section 10.5. Until both land, the security position above applies: the access code is not protected on 8883 and 6000. *Historical: this describes the MVP's scope. Both landed on 2026-09-17 (`29102cc`, `2ae6e39`); the access code is protected on all three ports.*
...
- The `[files]` table is written **before** `printers` in `config.toml`: a plain table after an array of tables would be read as part of the last printer. A config written before the table loads with the 5 GB default. The `[ui]` table of GUI polish decision O7 sits with it, before `printers`, and holds `zoom`: the window opens at the level it was left at, and a file carrying something it could not be used at (outside 0.5–3.0, or not a number) opens at 1.0.
...
- **The usage total is memoised, and walked off the UI thread.** The answer is a `read_dir` plus a `metadata` per file over the whole cache, so it never runs inside a frame (GUI polish E30): `main.rs` starts a thread for it at most once every 2 s and the line shows the last figure that landed. `usage_bytes_cached(max_age)` keeps the same 2 s rule for any caller that wants it inline, and `commit`, `evict_to`, `clear` and the sweeps forget the memo, so Clear cache and a finished download show at once rather than up to 2 s later. Clear cache itself also runs on a thread, and its button reads "Clearing…" until it is done.
...
| `src/tls.rs` (new) | `PrinterTls`, `PrinterCertVerifier`, `PrinterCertError`, `AnchoredConnector` / `AnchoredStream` and the per-connection records (5.3). Used by FTPS in the MVP, and by MQTT and the camera after issues #1 and #2. *Historical: both issues closed on 2026-09-17, so all three paths use it.* |
...
| `src/mqtt.rs`, `src/camera.rs` | Unchanged in the MVP: they keep `danger_*` and hand the access code to anyone who answers on 8883 and 6000 (issues #1 and #2, 10.5). *Historical: this describes the MVP's scope. Both were changed on 2026-09-17 (`29102cc`, `2ae6e39`) and now use `PrinterTls` and `AnchoredConnector` like FTPS.* |
...
- **Actions are always visible** on the selected tile and in the detail pane, not only on hover. In the detail pane they sit right under the facts, and are pinned to the foot of the pane only once the facts stop fitting (decision O3): "Save to PC" is on screen whatever the file carries, and never floating far below a short list of facts. An action the view would refuse — "Save to PC" or "Download & play" for a file already on its way — is disabled and says why on hover, never a click that starts nothing.
- **The list does O(listing) work only when the listing changes.** The row model (filtered, sorted, grouped), the status line's size sums and the selected entry are found once per `BrowserState::revision()` and kept for the frames that follow (GUI polish E16, E27). A transfer's progress does not rebuild them; a listing that lands, a letter typed in the filter or a window resized does.
- **The view has no repaint rate of its own.** Every counter that reads whole seconds asks for its own next second, and a tile waiting on its 500 ms prefetch gate asks for that moment (E32). An idle view with a listing repaints once a second, not ten times.
- **Opening a file in the player runs on a thread.** Walking a long AVI's index takes seconds, so the button reads "Opening…" until the player is ready (E30, E31).
...
- **Every control carries its name and its role** (GUI polish A9, A10): a surface says what it is called and whether it is the selected one, a glyph button says what it does rather than how it looks ("Cancel the download of …", "Dismiss"), the filter field carries "Filter by name", the transfer bars and the job bar carry their percentage, and Tab walks the header and the tool buttons in the order the eye reads them.
...
- **Loading:** skeleton tiles plus a "listing /cache…" caption. **Empty:** the reasons from 5.5. **Error:** a card with the 5.10 text and what can fix it (decision O14): Retry for a connection that dropped, "Edit printer" for a rejected access code, a bad address, a missing serial or a model refused by name, and "Clear cache" beside Retry when the volume filled up. Never an endless spinner, and never a retry that cannot work.
...
- **The virtualised list reserves heights derived from the style** (docs/gui-polish-guidelines.md 1.8, updated in the GUI polish pass): a list row, a companion line, a caption line, a month heading and a tile row each have a formula, nothing in a row wraps (a long note is truncated, whole on hover), and what a row painted is never fed back. The scroll range matches the content from the first frame, and one long note can no longer make every later row taller.
...
| Gate | What | Status |
|---|---|---|
| G1 | Phase 0 fixes committed (branch `development`, not merged yet) | done |
...
7. **`camera.rs:2` doc said "64-byte auth packet"** while the code sends 80 bytes. **Fixed** with issue #2. The same error was repeated in a LAN protocol note kept outside this repository, which is not tracked here.
